...
class GraspMVNeRFOptimizer(tf.keras.Model):
    def __init__(self, nerf_grasper, workspace_bounds, n_initial_guesses=32, n_images=3, clip_translation=False,
                 rotation_representation='quaternion',
                 **kwargs):
        super(GraspMVNeRFOptimizer, self).__init__(**kwargs)
    ...
    def call(self, inputs, training=False, mask=None):
        ...
        grasp_success = self.nerf_grasper.infer(
            (matrices, inputs[0][0], inputs[0][1], inputs[0][2]), self.n_initial_guesses, inputs[1])
