...
class DeltaNGFDataGenerator(DataGenerator):
    def __init__(self, dataset, workspace_bounds, n_views=1, batch_size=1, shuffle=True,
        ...
        super(DeltaNGFDataGenerator, self).__init__(dataset, batch_size=batch_size, shuffle=shuffle)
        ...
        if self.fixed_orientation is not None:
            ...
            n_r_fraction = 8
            self.n_negative = ((n_r_fraction - 1) * self.n_points_train) // n_r_fraction - self.future_poses
            self.n_r_negative = self.n_points_train - self.n_negative - self.future_poses

    # def get_valid_indices(self):
    #     # index is only valid if it's order is grater or equal to self.future_poses
    #     all_indices = np.arange(len(self.dataset))
    #     valid_indices = []
    #     for i in all_indices:
    #         if self.dataset.datasets['order'].read_sample(i) >= self.future_poses:
    ...
    def get_data_camera(self, batch):
        ...
        for i in batch:
            src_indices = np.random.choice(range(self.n_perspectives), size=self.n_views, replace=False)
            src_color = []
            src_extrinsics_inv = []
            src_intrinsics = []
    ...
    # def get_data_landscape_future(self, batch):
    #     input_translations = []
    #     input_quaternions = []
    #     targets = []
    #
    #     for i in batch:
    #
    #         target_poses = [self.dataset.datasets['target'].read_sample(i) for i in
    #                         range(i, i + self.future_poses)]
    #         negative_samples = [Affine.random(self.workspace_bounds).matrix for _ in range(self.n_negative)]
    #
    #         negative_r_transforms = [Affine.random(t_bounds=((-0.02, 0.02), (-0.02, 0.02), (-0.01, 0.02)),
    #                                                allow_zero_rotation=False) for _ in range(self.n_r_negative)]
    #         negative_r_samples = []
    #         if self.fixed_orientation is None:
    #             for j in range(self.n_r_negative):
    #                 target_idx = j % self.future_poses
    #                 negative_r_sample = target_poses[target_idx] @ negative_r_transforms[j].matrix
    #                 negative_r_samples.append(negative_r_sample)
    #         all_poses = [*target_poses, *negative_samples, *negative_r_samples]
    #         labels = np.concatenate((np.ones(self.future_poses), np.zeros(self.n_points_train - self.future_poses)),
    #                                 axis=0)
    #         input_translation = [Affine.from_matrix(pose).translation for pose in all_poses]
    #         input_quaternion = [Affine.from_matrix(pose).quat for pose in all_poses]
    ...
    #     input_quaternions = np.array(input_quaternions, dtype=np.float32)
    #     targets = np.array(targets)
    #     return input_translations, input_quaternions, targets
    ...
    def get_data_landscape_final(self, batch):
        ...
        for i in batch:
            ...
            negative_r_transforms = [Affine.random(t_bounds=((-0.01, 0.01), (-0.01, 0.01), (-0.01, 0.01)),
                                                   allow_zero_rotation=False) for _ in range(self.n_r_negative)]
            negative_r_samples = [target_pose @ r_transform.matrix for r_transform in negative_r_transforms]
            ...
            if self.rotation_representation == 'quaternion':
                input_rotation = [Affine.from_matrix(pose).quat for pose in all_poses]
            elif self.rotation_representation == '6d':
                input_rotation = [
                    np.concatenate([Affine.from_matrix(pose).rotation[:, 0], Affine.from_matrix(pose).rotation[:, 1]])
    ...
    def get_data_grad(self, batch):
        ...
        for i in batch:
            ...
            initial_index = np.random.randint(0, len(trajectory) - self.future_poses - 1)
            required_poses = trajectory[initial_index:initial_index + self.future_poses + 1]
            ...
            input_translations = [Affine.from_matrix(pose).translation for pose in augmented_poses]
            target_translations = [Affine.from_matrix(pose).translation for pose in augmented_targets]
            if self.rotation_representation == 'quaternion':
                input_rotations = [Affine.from_matrix(pose).quat for pose in augmented_poses]
                target_rotations = [Affine.from_matrix(pose).quat for pose in augmented_targets]
            elif self.rotation_representation == '6d':
                input_rotations = [
                    np.concatenate([Affine.from_matrix(pose).rotation[:, 0], Affine.from_matrix(pose).rotation[:, 1]])
    ...
    def get_data(self, batch):
        src_imagess, src_intrinsicss, src_extrinsics_invs = self.get_data_camera(batch)
        input_translations, input_rotations, targets = self.get_data_landscape_final(batch)
        translations, rotations, target_d_t, target_d_r = self.get_data_grad(batch)
        inputs = [
            input_translations,
            input_rotations,
            translations,
