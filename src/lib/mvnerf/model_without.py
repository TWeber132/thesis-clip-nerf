...
class MVVNeRFRenderer(tf.keras.Model):
    ...
    def call(self, inputs, training=False, mask=None):
        ...
        up_sampled_features = self.up_sample(visual_features)
        up_sampled_features = rearrange(
            up_sampled_features, '(b n) h w c -> b n h w c', b=self.batch_size)
        return self._call(inputs, self.n_rays_train, self.batch_size, up_sampled_features)
...
def render_view(model, src_colors, src_camera_configs, tgt_camera_config):
    ...
    up_sampled_features = model.up_sample(visual_features)
    up_sampled_features = rearrange(
        up_sampled_features, '(b n) h w c -> b n h w c', b=1)
