...
from lib.mvnerf.layers import MVResNetMLPNeRFEmbedding, RenderReadout, VisualFeatures, CombineCLIPVisualV4
...
class MVVNeRFRenderer(tf.keras.Model):
    def __init__(self, n_rays_train, n_rays_infer, n_views=2, n_samples=64, n_features=256,
        ...
        self.combine_clip_visual = CombineCLIPVisualV4()
...
def render_view(model, src_colors, src_camera_configs, tgt_camera_config):
    ...
    clip_textuals = tf.ones([tf.shape(src_images)[0], 1024])  # [(BN) 1024]
